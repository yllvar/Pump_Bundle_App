...
import { type PumpFun, IDL } from "./IDL"
...
export class PumpFunSDK {
    ...
    if (buyAmountSol > 0) {
      for (let i = 0; i < buyers.length; i++) {
        ...
        const buyAmountSolWithRandom =
          (buyAmountSol / BigInt(100)) * BigInt(randomPercent % 2 ? 100 + randomPercent : 100 - randomPercent)

        const globalAccount = await this.getGlobalAccount(commitment)

        // This is building buy transaction code
        // If you need it, contact me.
        let buyVersionedTx: VersionedTransaction
  ...
  // Update the createTokenMetadata method to use our Next.js API route:
